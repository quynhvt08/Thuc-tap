...
function EnhancedTableToolbar(props) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3001/entranceexamblocks', formData);
  ...
  const handleImportExcel = async (event) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        await Promise.all(processedData.map(item => axios.post('http://localhost:3001/entranceexamblocks', item)));
        const response = await axios.get('http://localhost:3001/entranceexamblocks');
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('http://localhost:3001/entranceexamblocks');
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:3001/entranceexamblocks/${selectedRowId}`);
  ...
  const handleEdit = async () => {
    ...
    try {
      await axios.put(`http://localhost:3001/entranceexamblocks/${selectedRowId}`, selectedRowData);
